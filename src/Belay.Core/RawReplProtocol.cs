...
public static class RawReplProtocol {
    ...
    public const byte ENTERRAW = 0x01;
    public const byte EXITRAW = 0x02;
    public const byte INTERRUPT = 0x03;
    public const byte EXECUTE = 0x04;
    public const byte RAWPASTE = 0x05;
    private const string NORMALPROMPT = ">";
    ...
    public static async Task<string> ExecuteCode(Stream stream, string pythonCode, CancellationToken cancellationToken = default) {
        try {
            // Enter raw mode
            await stream.WriteAsync(new byte[] { ENTERRAW }, cancellationToken);
            await WaitForPrompt(stream, NORMALPROMPT, cancellationToken);
            ...
            // Exit raw mode
            await stream.WriteAsync(new byte[] { EXITRAW }, cancellationToken);
    ...
    public static async Task<string> ExecuteCodeWithPaste(Stream stream, string pythonCode, CancellationToken cancellationToken = default) {
        try {
            ...
            await stream.WriteAsync(new byte[] { RAWPASTE, 0x01 }, cancellationToken);
