...
public class SimpleRawRepl : IDisposable
{
    ...
    private const byte CTRL_C = 0x03; // Interrupt
    private const byte CTRL_D = 0x04; // Execute/End data
    private const byte CTRL_E = 0x05; // Raw-paste mode prefix

    // Raw-paste mode initialization sequence
    private static readonly byte[] RAW_PASTE_INIT = [CTRL_E, (byte)'A', CTRL_A];
    ...
    public async Task EnterRawReplAsync(bool softReset = true, int timeoutSeconds = 10, CancellationToken cancellationToken = default)
    {
        ...
        await this.stream.WriteAsync(new byte[] { (byte)'\r', CTRL_C }, cancellationToken);
        ...
        await this.stream.WriteAsync(new byte[] { (byte)'\r', CTRL_A }, cancellationToken);
        await this.stream.FlushAsync(cancellationToken);
        ...
        if (softReset)
        {
            ...
            await this.stream.WriteAsync(new byte[] { CTRL_D }, cancellationToken);
    ...
    public async Task ExitRawReplAsync(CancellationToken cancellationToken = default)
    {
        ...
        await this.stream.WriteAsync(new byte[] { (byte)'\r', CTRL_B }, cancellationToken);
        await this.stream.FlushAsync(cancellationToken);
    ...
    public async Task<RawReplResult> ExecuteAsync(string command, int timeoutSeconds = 10, CancellationToken cancellationToken = default)
    {
        ...
        if (!this.atPrompt)
        {
            ...
            this.logger.LogDebug("Prompt check response: '{Data}' (length: {Length})", 
    ...
    private async Task ExecuteWithRawPasteAsync(byte[] commandBytes, CancellationToken cancellationToken)
    {
        ...
        await this.stream.WriteAsync(RAW_PASTE_INIT, cancellationToken);
        await this.stream.FlushAsync(cancellationToken);
    ...
    private async Task<RawReplResult> ReadExecutionResultAsync(int timeoutSeconds, bool wasRawPaste, CancellationToken cancellationToken)
    {
        ...
        this.logger.LogDebug("Normal output: '{Output}' (ends with EOF: {EndsWithEOF})", normalOutput, normalOutput.EndsWith("\x04"));
        
        if (!normalOutput.EndsWith("\x04"))
        {
        ...
        this.logger.LogDebug("Error output: '{Output}' (ends with EOF: {EndsWithEOF})", errorOutput, errorOutput.EndsWith("\x04"));
        
        if (!errorOutput.EndsWith("\x04"))
        {
        ...
        this.logger.LogDebug("Execution result - Type: {ErrorType}, Success: {Success}, Output: '{Output}', Error: '{Error}'", 
        ...
        {
            this.logger.LogWarning("Execution error classified as {ErrorType}: {DiagnosticInfo}. Suggested action: {SuggestedAction}",
    ...
    private async Task<string> ReadUntilAsync(string ending, int timeoutSeconds, CancellationToken cancellationToken)
    {
        return await this.ReadUntilAsync(ending, timeoutSeconds, "general", cancellationToken);
    }
    ...
    private async Task<string> ReadUntilAsync(string ending, int timeoutSeconds, string operationType, CancellationToken cancellationToken)
    {
        ...
        this.logger.LogTrace("Starting ReadUntilAsync for '{Ending}' with {Timeout}s timeout (operation: {Operation})", 
        ...
        while (DateTime.UtcNow - startTime < actualTimeout)
        {
            ...
            try
            {
                ...
                {
                    this.logger.LogTrace("Successfully read until '{Ending}' in {Duration}ms", 
                        ending.Replace("\r", "\\r").Replace("\n", "\\n"), 
        ...
        this.logger.LogWarning("ReadUntilAsync timed out after {Timeout}s waiting for '{Ending}' (operation: {Operation}, received: {ReceivedLength} chars)", 
    ...
    {
        if (!this.inRawRepl || !this.stream.CanWrite)
        {
            return;
        }
    ...
    private async Task ExitRawReplGracefullyAsync(CancellationToken cancellationToken)
    {
        ...
        await this.stream.WriteAsync(new byte[] { CTRL_B }, cancellationToken);
        await this.stream.FlushAsync(cancellationToken);
