...
internal class AdaptiveChunkOptimizer
{
    ...
    private const int MIN_CHUNK_SIZE = 64;      // Minimum chunk size (64 bytes)
    private const int MAX_CHUNK_SIZE = 4096;    // Maximum chunk size (4KB)
    private const double THROUGHPUT_THRESHOLD = 0.95; // 95% threshold for increasing chunk size
    private const int SAMPLE_SIZE = 5;          // Number of samples for stability
    ...
    public AdaptiveChunkOptimizer(int initialChunkSize, ILogger logger)
    {
        ...
        this.initialChunkSize = Math.Max(MIN_CHUNK_SIZE, Math.Min(initialChunkSize, MAX_CHUNK_SIZE));
        this.currentChunkSize = this.initialChunkSize;
        
        this.logger.LogDebug("Initialized adaptive chunk optimizer with initial size: {InitialSize} bytes", this.currentChunkSize);
    }
    ...
    public void RecordTransfer(int bytesTransferred, TimeSpan duration)
    {
        ...
        this.logger.LogTrace("Transfer recorded: {Bytes} bytes in {Duration}ms, throughput: {Throughput:F1} bytes/s, avg: {AvgThroughput:F1} bytes/s", 
        ...
        if (this.measurementCount >= SAMPLE_SIZE)
        {
    ...
    private void OptimizeChunkSize(double currentThroughput)
    {
        ...
        if (currentThroughput > this.averageThroughput * THROUGHPUT_THRESHOLD && 
            this.currentChunkSize < MAX_CHUNK_SIZE)
        {
            ...
            this.currentChunkSize = Math.Min(MAX_CHUNK_SIZE, this.currentChunkSize + increase);
        }
        ...
        this.currentChunkSize = Math.Max(MIN_CHUNK_SIZE, Math.Min(MAX_CHUNK_SIZE, this.currentChunkSize));
        
        if (oldChunkSize != this.currentChunkSize)
        {
            this.logger.LogDebug("Optimized chunk size: {OldSize} -> {NewSize} bytes (throughput: {Throughput:F1} bytes/s)", 
