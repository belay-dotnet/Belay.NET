...
public class BelayHealthCheck : IHealthCheck {
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default) {
        try {
            ...
            if (await CheckSessionManager(data, warnings, cancellationToken).ConfigureAwait(false)) {
            ...
            _logger.LogError("Belay health check failed with multiple issues: {Warnings}", string.Join(", ", warnings));
    ...
    private async Task<bool> CheckSessionManager(Dictionary<string, object> data, List<string> warnings, CancellationToken cancellationToken) {
        try {
            var stats = await _sessionManager.GetSessionStatsAsync(cancellationToken).ConfigureAwait(false);
            ...
            _logger.LogWarning(ex, "Session manager health check failed");
