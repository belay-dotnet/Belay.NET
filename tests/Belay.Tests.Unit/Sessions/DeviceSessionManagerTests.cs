...
namespace Belay.Tests.Unit.Sessions {
    ...
    public class DeviceSessionManagerTests {
        ...
        public void SetUp() {
            ...
            mockLogger = new Mock<ILogger<DeviceSessionManager>>();
            mockCommunication = new Mock<IDeviceCommunication>();

            mockLoggerFactory.Setup(f => f.CreateLogger(typeof(DeviceSessionManager).FullName!))
                .Returns(mockLogger.Object);
            mockLoggerFactory.Setup(f => f.CreateLogger(typeof(DeviceSession).FullName!))
                .Returns(new Mock<ILogger<DeviceSession>>().Object);
            mockLoggerFactory.Setup(f => f.CreateLogger(typeof(ResourceTracker).FullName!))
                .Returns(new Mock<ILogger<ResourceTracker>>().Object);
            mockLoggerFactory.Setup(f => f.CreateLogger(typeof(ExecutorContext).FullName!))
                .Returns(new Mock<ILogger<ExecutorContext>>().Object);
            mockLoggerFactory.Setup(f => f.CreateLogger(typeof(DeviceContext).FullName!))
                .Returns(new Mock<ILogger<DeviceContext>>().Object);
            mockLoggerFactory.Setup(f => f.CreateLogger(typeof(DeviceCapabilities).FullName!))
            ...
            sessionManager = new DeviceSessionManager(mockLoggerFactory.Object);
        }
        ...
        public void Constructor_WithValidLoggerFactory_SetsStateToInactive() {
            // Act & Assert
            sessionManager.State.Should().Be(DeviceSessionState.Inactive);
            sessionManager.CurrentSessionId.Should().BeNull();
        }
        ...
        public async Task CreateSessionAsync_WithValidCommunication_ReturnsSession() {
            // Arrange
            mockCommunication.Setup(c => c.ExecuteAsync<string>(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            ...
            sessionManager.CurrentSessionId.Should().Be(session.SessionId);
        }

        [Test]
        public async Task CreateSessionAsync_WithNullCommunication_ThrowsArgumentNullException() {
            // Act & Assert
            var act = async () => await sessionManager.CreateSessionAsync(null!);
        ...
        public async Task GetOrCreateSessionAsync_WhenNoCurrentSession_CreatesNewSession() {
            ...
            // Act
            var session = await sessionManager.GetOrCreateSessionAsync(mockCommunication.Object);
        ...
        public async Task GetOrCreateSessionAsync_WhenCurrentSessionExists_ReturnsExistingSession() {
            ...
            var firstSession = await sessionManager.CreateSessionAsync(mockCommunication.Object);

            // Act
            var secondSession = await sessionManager.GetOrCreateSessionAsync(mockCommunication.Object);

            // Assert
            secondSession.Should().BeSameAs(firstSession);
            sessionManager.CurrentSessionId.Should().Be(firstSession.SessionId);
        }
        ...
        public async Task ExecuteInSessionAsync_WithAction_ExecutesSuccessfully() {
            ...
            // Act
            await sessionManager.ExecuteInSessionAsync(
                mockCommunication.Object,
                session => {
                    session.Should().NotBeNull();
                    session.IsActive.Should().BeTrue();
        ...
        public async Task GetSessionStatsAsync_ReturnsCorrectStatistics() {
            ...
            // Act
            var initialStats = await sessionManager.GetSessionStatsAsync();
            await sessionManager.CreateSessionAsync(mockCommunication.Object);
            var afterCreateStats = await sessionManager.GetSessionStatsAsync();
        ...
        public async Task DisposeAsync_CleansUpSessionsAndResources() {
            ...
            // Assert
            sessionManager.State.Should().Be(DeviceSessionState.Disposed);
            session.IsActive.Should().BeFalse();
        }
        ...
        public async Task DeviceCapabilities_WhenDetected_AreExposed() {
            ...
            // Assert
            sessionManager.Capabilities.Should().NotBeNull();
            sessionManager.Capabilities!.FirmwareVersion.Should().NotBeNullOrEmpty();
            sessionManager.Capabilities.DeviceType.Should().NotBeNullOrEmpty();
        }
    }
}
