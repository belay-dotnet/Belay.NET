...
namespace Belay.Samples;

/// <summary>
/// Demonstrates comprehensive device subclassing using all Belay.NET attributes.
/// This environmental monitoring device showcases the attribute-based programming
/// model for MicroPython devices.
/// </summary>
/// <remarks>
/// <para>
/// This sample illustrates the complete lifecycle of a device-specific class:
/// <list type="bullet">
/// <item><description>[Setup] methods initialize hardware and global state</description></item>
/// <item><description>[Task] methods provide synchronous device operations</description></item>
/// <item><description>[Thread] methods enable background monitoring</description></item>
/// <item><description>[Teardown] methods ensure clean resource cleanup</description></item>
/// </list>
/// </para>
/// <para>
/// The environmental monitor reads temperature, humidity, and light sensors
/// while demonstrating advanced patterns like background data collection,
/// calibration management, and graceful error handling.
/// </para>
/// </remarks>
/// <example>
...
/// <code>
/// // Connect to device
/// var monitor = new EnvironmentMonitor("serial:COM3");
...
/// await monitor.StartContinuousMonitoringAsync(intervalMs: 5000);
/// 
/// // Clean disconnect
/// await monitor.DisconnectAsync();
/// </code>
/// <para><strong>With Logging and Error Handling</strong></para>
/// <code>
...
/// try
/// {
///     var monitor = new EnvironmentMonitor("serial:/dev/ttyACM0", loggerFactory);
///     await monitor.ConnectAsync();
///     
///     // Calibrate sensors
///     await monitor.CalibrateSensorsAsync();
///     
///     // Get readings with retry on failure
///     var reading = await monitor.GetCurrentReadingAsync();
...
///     await monitor.StartContinuousMonitoringAsync(1000);
///     
...
/// </code>
/// </example>
public class EnvironmentMonitor : IDisposable
{
    private readonly Device _baseDevice;
    ...
    public EnvironmentMonitor(string connectionString, ILoggerFactory? loggerFactory = null)
    {
        _baseDevice = Device.FromConnectionString(connectionString, loggerFactory);
    }

    /// <summary>
    /// Connect to the environmental monitoring device.
    /// </summary>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    public new async Task ConnectAsync(CancellationToken cancellationToken = default)
    {
        await _baseDevice.ConnectAsync(cancellationToken);
    }

    /// <summary>
    /// Disconnect from the environmental monitoring device.
    /// </summary>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    public new async Task DisconnectAsync(CancellationToken cancellationToken = default)
    {
        await _baseDevice.DisconnectAsync(cancellationToken);
    }
    ...
    private async Task<string> ExecuteAsync(string code, CancellationToken cancellationToken = default)
    {
        return await _baseDevice.ExecuteAsync(code, cancellationToken);
    }

    /// <summary>
    /// Execute Python code on the device with typed result.
    /// </summary>
    /// <typeparam name="T">Expected result type.</typeparam>
    /// <param name="code">Python code to execute.</param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>Typed execution result.</returns>
    private async Task<T> ExecuteAsync<T>(string code, CancellationToken cancellationToken = default)
    {
        return await _baseDevice.ExecuteAsync<T>(code, cancellationToken);
    }
    ...
    /// </summary>
    [Setup(Order = 3, Critical = false)] // Non-critical setup
    private async Task InitializeMonitoringStateAsync()
    {
        await ExecuteAsync(@"
    ...
    [Task]
    public async Task<EnvironmentReading> GetCurrentReadingAsync()
    {
        return await ExecuteAsync<EnvironmentReading>(@"
            import json
            import time
    ...
    [Task(TimeoutMs = 30000)] // Extended timeout for calibration
    public async Task CalibrateSensorsAsync(float referenceTemp = 25.0f, float referenceHumidity = 50.0f)
    {
        await ExecuteAsync($@"
            import json
            import time
    ...
    /// </summary>
    [Task(Cache = false)] // Always get fresh diagnostics
    ...
    public async Task StartContinuousMonitoringAsync(int intervalMs = 5000)
    {
        await ExecuteAsync($@"
            import _thread
            import time
    ...
    /// </summary>
    [Thread(Name = "health_watchdog", Priority = ThreadPriority.High, MaxRuntimeMs = 300000)]
    public async Task StartHealthWatchdogAsync()
    {
        await ExecuteAsync(@"
            import _thread
            import time
    ...
    /// </summary>
    [Teardown(Order = 2, IgnoreErrors = true)] // Non-critical - don't fail disconnect if this fails
...
public class EnvironmentReading
{
    ...
    /// <summary>
    /// Timestamp when the reading was taken (device ticks).
    /// </summary>
